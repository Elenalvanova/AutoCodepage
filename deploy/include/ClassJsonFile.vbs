...
'
' This file is part of the deployment system of the AutoCodepage plugin for
' Notepad++.
'
...
Class clsJsonFile
  ...
  Private Function PrintJsonDic(ByRef dicDataStore, strIndent, bolClean)
    ...
    If strLine <> "" Then strLine = strLine & vbNewLine
  ...
  Private Function PrintJsonArr(ByRef arrDataStore, strIndent, bolClean)
    ...
    For Each varValue In arrDataStore
      ...
      If IsObject(varValue) Then
        strLine = strLine & strIndent & JsonArrPrefix(bolClean, intLocalIndentLen, intCnt) & "{" & vbNewLine & _
                            PrintJsonDic(varValue, strIndent & strLocalIndent & String(intIndent, " "), bolClean) & _
                            strIndent & strLocalIndent & "}"

      ElseIf IsArray(varValue) Then
        strLine = strLine & strIndent & JsonArrPrefix(bolClean, intLocalIndentLen, intCnt) & "[" & vbNewLine & _
                            PrintJsonArr(varValue, strIndent & strLocalIndent & String(intIndent, " "), bolClean) & _
                            strIndent & strLocalIndent & "]"
